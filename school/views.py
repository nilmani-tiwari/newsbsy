...
def logoutUser(request):
    Profile.objects.get_or_create(user=request.user.id)
    obj=Profile.objects.filter(user=request.user.id)
    obj.update(is_online=False)
    logout(request)
...
def parents(request):
	...
	msg=parents_msg.objects.values().filter(user_name=request.user.username)
   
	for i in child:
		i.update(ParentsMaster.objects.values().filter(user_id=i["parents_id"]).first())

	d={"child":child,"data":msg}
	return render(request,'SchoolDesign/index4.html',d)


@login_required(login_url='login')
def all_schools(request):
    data=SchoolMaster.objects.values()
    students_data=StudentMaster.objects.values()
    staff_data=StaffMaster.objects.values()
    atten=Attendance.objects.values()
    ...
    for i in data:
        stranth=students_data.filter(school_code=i["school_code"]).count()+staff_data.filter(staff_school_code=i["school_code"]).count()
        i.update({"stranth":stranth})
        if stranth<30:
            demo_count+=1
        all_atten_today=atten.filter(school_code=i["school_code"],date=date).count()
        if all_atten_today>1:
            active_count+=1
        try:
            ratio=int((int(all_atten_today)/int(stranth))*100    )        
        except:
            ratio=0 
        inactive_count=int(school_count)-int(active_count)
        i.update({"atten_ratio":ratio})    
        i.update({"atten_today":all_atten_today})
    return render(request,"sbsy_admin/all_schools.html",{"data":data,"school_count":school_count,"demo_count":demo_count,"active_count":active_count,"inactive_count":inactive_count})
	...
	for i in data:
		if mahina==int(month):
			for j in range(1,today+1):
				...
				attn = Attendance.objects.values().filter(user_name=i["user_name"],school_code=school_code, date=date)
	...
	for i in data:
		if mahina==int(month):
			for j in range(1,today+1):
				...
				attn = Attendance.objects.values().filter(user_name=i["staff_user_name"] ,school_code=school_code, date=date)
...
def manual_atten_all_staff(request):
    d={}
    school_code=request.user.last_name
    mediums = MediumMaster.objects.values('medium_name').filter(school_code=school_code)
    d={"mediums":mediums}
    date = datetime.datetime.now()
    staff = StaffMaster.objects.values().filter(staff_school_code=school_code,staff_medium_name="English")
    n = 1
    absent_staff = []
    for i in staff:
        if Attendance.objects.values().filter(user_name=i["staff_user_name"], school_code=school_code,date=date).exists() == False:
            if 1==1:
                absent_staff.append(i)  
            else:pass
        if n < 10:
            i.update({'index': '0' + str(n)})
        else:
            i.update({'index': n})
        n += 1
    d.update({"x": absent_staff,"staff":staff})
    if request.method == "POST":
        medium_name = request.POST.get('medium_name')
        if medium_name=="":
            medium_name="English"
        ...
        manual_atten(request,school_code,date,ch)
        
        d.update({"x": absent_staff})
        
        return render(request, 'manual_atten_all_staff.html', d)
            
    return render(request, 'manual_atten_all_staff.html',d)
    

def manual_atten_all(request):
    d={}
    school_code = request.user.last_name
    ...
    d={"mediums":mediums}
    date = datetime.datetime.now()
    students = StudentMaster.objects.values().filter(school_code=school_code,medium_name="English")
    n = 1
    absent_student = []
    for i in students:
        if Attendance.objects.values().filter(user_name=i["user_name"], school_code=school_code,date=date).exists() == False:
            try:
                i.update({"father_mobile_number": ParentsMaster.objects.get(user_id=i["parents_id"]).Parents_mobile})
    ...
    if request.method == "POST":
        ...
        #absentmsg(ch)
        #sbsy_absent_msg(request,school_code,date,ch)
        #absent_msg(request, school_code, date, ch)
        manual_atten(request,school_code,date,ch)
        
        d.update({"x": absent_student})
        
        return render(request, 'manual_atten_all.html', d)
...
def forget_password(request):
	if request.method == "POST":
		username = request.POST.get('username')
		user = User.objects.values().filter(username=username, is_staff=1, is_active=1).first()
		if user is not None:
			print(user, "***************************************")
			if User.objects.filter(groups=2, username=user['username']).exists():  # admin
				x = SchoolMaster.objects.values().filter(email=user['username']).first()
				print(x["email"])
				#email="sachinparashar502@gmail.com"
				email="krishanadthservices@gmail.com"
				message="your password is"+x["password"]
				#import smtplib
				if 1==1:
					if email is not None:
				# 		server = smtplib.SMTP_SSL("smtp.gmail.com", 465)
				# 		server.login("indiasbsy@gmail.com", "Sbsy@sbsy")
				# 		server.sendmail("indiasbsy@gmail.com",email,message)
				# 		server.quit
						send_mail('password reset',message,settings.EMAIL_HOST_USER,[email],fail_silently=False)
						messages.info(request, "password has sent to your email id: ****"+email[4::])
						return redirect("home")
						
					else:
						messages.info(request, "Register your email id")
				else:
					messages.info(request, "please contact to coustomer care")
# number=int(x["mobile"])
# personalsms(request,message,number)
# print(int(x["mobile"]))
# # x.update(password=Confirm_New_Password)
#print("password changed in SchoolMaster******************admin*********")
# 
			elif User.objects.filter(groups=4,  username=user['username']).exists():  # student
				x = StudentMaster.objects.values().filter(user_name=user['username'])
				email=x["email"]
				message="your password is"+x["password"]
				import smtplib
				try:
					...
					messages.info(request, "please contact to coustomer care")
			elif User.objects.filter(groups=3,  username=user['username']).exists():  # teacher
				x = StaffMaster.objects.values().filter(email=user['username']).first()
				email=x["staff_email"]
				message="your password is"+x["password"]
				import smtplib
				try:
					...
					messages.info(request, "please contact to coustomer care")
			elif User.objects.filter(groups=5,  username=user['username']).exists():  # parents
				x = ParentsMaster.objects.values().filter(email=user['username']).first()
				email=x["Parents_email"]
				message="your password is"+x["password"]
				import smtplib
				try:
					if email is not None:
			...
			messages.info(request, "No search results! Please try again with other information.")
...
def student_idcard(request,id):
    ...
    student=StudentMaster.objects.all().filter(school_code=school_code)
    student=StudentMaster.objects.values().filter(school_code=school_code).order_by('class_name')
...
@login_required(login_url='login')
def staff_idcard(request,id):
    school_code = request.user.last_name
    student=""
    school_info=SchoolMaster.objects.get(school_code=school_code)
    student=StudentMaster.objects.all().filter(school_code=school_code)
    staff=StaffMaster.objects.values().filter(staff_school_code=school_code).order_by('staff_class_name','staff_division_name')
    staff_html_file="SchoolDesign/id_card_view/staff_id_view"+str(id)+".html"
    for i in staff:pass
        #i.update(ParentsMaster.objects.values().filter(user_id=i["parents_id"]).first())
    return render(request,staff_html_file,{"data":staff,"school_info":school_info})
...
def add_school(request):
    if request.method == 'POST':
        ...
        registration_number= request.POST.get('registration_number')
        affiliated_by = request.POST.get('affiliated_by')
        email = request.POST.get('email').lower()#-----------required
...
def school_data_update(request, machine_no,):
    ...
    if "nilmani12345" == "nilmani12345":
        ...
        if school_machine_update.objects.values().filter(machine_no=machine_no).exists():
            data = school_machine_update.objects.values().filter(machine_no=machine_no)   #.first()
...
def delete_school():pass

@unauthenticated_user
def school_login(request,username,password):
    logout(request)
...
@login_required(login_url='login')
def parent_absent_msg(request):
    return redirect('parents')

#Alok Badarpur, [05.05.21 21:30]
@login_required(login_url='login')
def home_work(request):
    ...
    d={}
    if User.objects.filter(groups=2, username=user_name).exists():
        x = class_homework.objects.values().filter(school_code=school_code,).order_by('-created_on').distinct()
        d = {"x": x, "mediums": mediums, "classes": classes, "division": division}
    elif User.objects.filter(groups=3, username=user_name).exists():
        x = class_homework.objects.values().filter(school_code=school_code, created_by=user_name).order_by('-created_on')
        d = {"x": x, "mediums": mediums, "classes": classes, "division": division}
    elif User.objects.filter(groups=4, username=user_name).exists():
        student=StudentMaster.objects.values().filter(user_name=user_name)
        h=homework_submit.objects.values().filter(created_by=user_name)
        l1=[]
        for j in h:
            l1.append(int(j["class_homework_id"]))
        for i in student:
            x = class_homework.objects.values().filter(school_code=school_code,medium=i['medium_name'],classes=i['class_name'],section=i['division_name']).order_by('-created_on')
            for p in x:
                if p["id"]  in l1:
                    p.update({"submit1":"fas fa-check text-success"})
                else:
                    p.update({"submit2":"fas fa-times text-danger"})

            d = {"x": x, "mediums": mediums, "classes": classes, "division": division,"h":h}
    else:
        d = {}
    ...
    if request.method == 'POST':
        ...
        image=request.FILES['image']
        hw, created = class_homework.objects.get_or_create( school_code=school_code,medium=medium_name,classes=class_name,section=division_name,subject=subject,message=message,image=image,created_by=request.user.username)
        hw.save()
        return redirect('home_work')
    return render(request, 'homework/view_homework.html',d)


@login_required(login_url='login')
def homework_delete(request,id):
    hw= class_homework.objects.get(id=id)
    hw.delete()
    messages.success(request, "Successfully Deleted")
    return redirect('home_work')
...
def s(request,id):
    if request.method=="POST":
        ...
        hw, created = homework_submit.objects.get_or_create(school_code=school_code,image=image1,class_homework_id=id,created_by=request.user.username)
        hw.save()
        return redirect('home_work')
    return redirect('home_work')
...
@login_required(login_url='login')
def homework_check(request,id):
    # x = class_homework.objects.values().filter(school_code=school_code, id=id)
    h = homework_submit.objects.values().filter(class_homework_id=id).order_by('-created_on')
    for i in h:
        x=StudentMaster.objects.values().filter(user_name=i["created_by"]).first()
        i.update({"clas":x["class_name"]})
        i.update({"section": x["division_name"]})
        i.update({"name": x["Full_name"]})

    d = {"h":h}
    return render(request, 'homework/submit_homework.html',d)
...
def notification(request):
    return render(request,'notification_tests.html')#this is sachin
...
def pay(request):
    if request.method == "POST":
        ...
        client = razorpay.Client(
            auth=("rzp_test_mT7vMSWtT1Rzf3", "YvOR84W5Y1PT3NeVT0NtfwKz"))
