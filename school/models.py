...
class ParentsMaster(models.Model):
    user=models.OneToOneField(User,on_delete=models.CASCADE,primary_key=True)
    #parents_user_name= models.CharField(max_length=200, blank=True, null=True)
...
class StaffMaster(models.Model):
    ...
    staff_category = models.CharField(max_length=200, blank=True, null=True)
    password=models.CharField(max_length=200, blank=True, null=True)
    created_on = models.DateTimeField(default=datetime.now, blank=True)
    created_by = models.IntegerField(blank=True, null=True)
    def save(self, *args, **kwargs):
        self.staff_user_name = str(self.staff_user_name).lower()
        self.staff_First_name=(self.staff_First_name).lower()
        self.staff_Last_name = str(self.staff_Last_name).lower()
        self.staff_email = str(self.staff_email).lower()
        self.staff_Address = str(self.staff_Address).lower()

        return super(StaffMaster, self).save(*args, **kwargs)
    def __str__(self):
        return self.staff_First_name+" "+str( self.staff_school_code)+" "+str(self.staff_user_name)
...
class school_machine_update(models.Model):
    ...
    created_on = models.DateTimeField(default=datetime.now, blank=True)
    def __str__(self):
        return str(self.machine_no)+" "+str(self.machine_status)+" "+str(self.school_code)
...
class parents_msg(models.Model):
    user_name = models.CharField(max_length=200, blank=True, null=True)
    sender_user_name=models.CharField(max_length=200, blank=True, null=True)
    school_code = models.CharField(max_length=200, blank=True, null=True)
    date=models.CharField(max_length=64, blank=True, null=True)
    school_code = models.CharField(max_length=200, blank=True, null=True)
    message=models.CharField(max_length=200, blank=True, null=True)
...
class homework_submit(models.Model):
    ...
    school_code = models.CharField(max_length=200, blank=True, null=True)
    image = models.ImageField(upload_to='images/', blank=True, null=True)
    class_homework_id=models.CharField(max_length=200, blank=True, null=True)
    created_by = models.CharField(max_length=200, blank=True, null=True)
    created_on = models.DateTimeField(default=datetime.now, blank=True)


class class_homework(models.Model):
    id = models.AutoField(primary_key=True)
    school_code = models.CharField(max_length=200, blank=True, null=True)
...
class Profile(models.Model):
    ...
    def __str__(self):
        if self.is_online:
            ...
            status="offline"
        return str(status)+" --> "+ str(self.user)+"  &&&&"+str(self.last_activity)
